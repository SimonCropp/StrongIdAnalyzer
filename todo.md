...
Suggested order: the crashes and the fixes that edit the wrong code (items 1–5), then the build-breaking SIA004 cases (items 13–14).
...
- [ ] **1. A local whose initializer refers back to itself overflows the stack.**
...
- [ ] **2. Wrapper recognition overflows the stack on valid code** (only with `strongidanalyzer.infer_wrapper_ids = true`).
  - Repro:
    ```cs
...
- [ ] **3. A `null` params array throws `NullReferenceException`** (reported as AD0001).
...
- [ ] **4. The rename fix renames the wrong symbol when the declaration is in another file.**
...
- [ ] **5. The add and change fixes do nothing when the diagnostic's syntax tree is stale.**
...
- [ ] **6. The SIA005 fix deletes comments above the attribute.**
...
- [ ] **7. The generic-form fix ignores arity.**
...
- [ ] **8. SIA002 fires on lambda parameters, and its fix does not compile.**
...
- [ ] **9. The rename fix is wrong for tags that start with a lower-case letter.**
...
- [ ] **10. Tuple elements get the wrong fix target.**
...
- [ ] **11. Nested lambdas bind the outer parameter to the inner collection.**
...
- [ ] **12. A local keeps its initializer's tag after reassignment.**
...
- [ ] **13. SIA004 (an error) fires for private fields.**
...
- [ ] **14. SIA005 asks to remove the `[Id]` that prevents SIA004.**
...
- [ ] **15. SIA005 ignores tags inherited from a base member.**
...
- [ ] **16. SIA003 fires on members of a generic type used with a concrete type argument.**
...
- [ ] **17. The same-element-type rule overrides an explicit `[return: Id]`.**
  - Repro:
    ```cs
    [return: Id("Order")]
...
- [ ] **18. Generic methods lose tags declared on the interface they implement.**
  - Repro:
    ```cs
...
- [ ] **19. `product.Id` gets no tag when `Id` is declared on a `Product` in a referenced assembly.**
...
- [ ] **20. A `[StrongIdIndex]` hit drops receiver tags from derived types in the consuming project.**
...
- [ ] **21. Tag widening ignores the suppression lists and scans every reference.**
...
- [ ] **22. `_id = id` in a constructor reports SIA002; `Id = id` does not.**
