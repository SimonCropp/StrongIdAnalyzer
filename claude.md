...
Opt-in **suffix inference** (`.editorconfig`, `strongidanalyzer.infer_suffix_ids = true`, default off) extends rule 2 for properties, fields, and parameters: walk back from the trailing `Id` through upper-case boundaries shortest-first, and take the first candidate that matches a tag already present in the compilation (any rule 1 / rule 2 / explicit attribute). So `sourceProductId` → `"Product"` when `Product` has a conventional `Id`; `templateExternalObjectId` → `"ExternalObject"` when `"Object"` is unknown but `"ExternalObject"` is known; `productOrderId` with both known resolves to `"Order"` (shortest match). On miss of every candidate shy of the whole prefix, falls through to the normal whole-name rule. Known-tag set is computed lazily via `Lazy<ImmutableHashSet<string>>` on `Config`, populated by walking `compilation.Assembly.GlobalNamespace` on first use. Hook point: `GetIdWithInheritance`, before the `TryGetConventionName` fallback.
