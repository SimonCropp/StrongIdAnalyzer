...
2. **Rename the source** so the naming convention infers the id: `Raw` → `OrderId`, a parameter `value` → `orderId`. Same effect, no attribute. The rename is only offered when the convention reads the new name back as the same tag — for a tag that starts lower-case, such as `[Id("customer")]`, it never can, because the convention upper-cases the first character.
 ...
 * The source is a constructor / setter parameter whose name corresponds to the target property **or field**: `Tenant(string id) => Id = id;` and `Tenant(string id) => _id = id;` read the same way — a field's underscore prefix is punctuation, not part of the name being matched. The name already expresses the binding, so an extra attribute would be noise.
 * The source is a lambda parameter. C# only accepts an attribute there inside a parenthesized parameter list, and not at all inside an expression tree (CS8972), so there is no fix to offer — tag the collection being queried instead.
 * The source is a tuple element. Its declaration lives inside a tuple *type*, so there is nowhere to put the attribute that would describe that element alone.
