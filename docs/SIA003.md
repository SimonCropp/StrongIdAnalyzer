...
2. **Rename the target** so the naming convention infers the id: `value` → `orderId`. Same effect, no attribute. The rename is only offered when the convention reads the new name back as the same tag — for a tag that starts lower-case, such as `[Id("customer")]`, it never can, because the convention upper-cases the first character.
 ...
 * **Unconstrained generic type parameters (`T`), and constructed generics still containing one (`TestEntity<T>`, `List<T>`, `Dictionary<Guid, T>`, `T[]`)** — identity methods, container helpers, generic wrappers. The target can't carry a domain tag while T is unbound, so SIA003 would have no place to land a fix. This is judged on the *declaration*, so it covers a member reached through a closed receiver too: `box.Content = customerId` where `box` is a `Box<Guid>` would land the fix on `Box<T>.Content`, binding the tag to every other closing of `T` as well.
 * **Lambda parameters and tuple elements** — neither has a declaration an attribute can be written against: a lambda parameter needs a parenthesized parameter list and is refused outright inside an expression tree, and a tuple element is declared inside a tuple *type*.
