...
## Qualified names and suffix inference

A qualifier word in a name becomes part of the id under the whole-name rule: `templateExternalObjectId` is `"TemplateExternalObject"`, not `"ExternalObject"`. Retagging the target to `"TemplateExternalObject"` is the wrong fix for that. When [suffix inference](/readme.md#suffix-inference-opt-in) is off and turning it on would resolve the mismatch, the message says so:

```
SIA001: parameter 'templateExternalObjectId' of 'Service.Copy' is [Id("TemplateExternalObject")] and flows to parameter 'externalObjectId' of 'Service.Get', which is [Id<ExternalObject>]. Fix: apply [Id("TemplateExternalObject")] to parameter 'externalObjectId' of 'Service.Get' (line 4), or pass a value tagged [Id<ExternalObject>]. Alternatively, set strongidanalyzer.infer_suffix_ids = true in .editorconfig, which infers [Id<ExternalObject>] from 'templateExternalObjectId'.
```

The hint appears only when the ids the suffix rule would infer clear the mismatch, and names every side it re-reads (`sourceProductId` into `targetProductId` needs both). To keep the option off, tag the qualified member explicitly: `[Id<ExternalObject>] int templateExternalObjectId`.
